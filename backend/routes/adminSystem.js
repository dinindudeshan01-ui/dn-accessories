...
// GET /api/admin/system/audit?limit=100&entity=product&action=DELETE
...
// GET /api/admin/system/audit/stats
...
// Body: { targets: ['orders','expenses','cogs','products','suppliers'], confirm: 'RESET' }
router.post('/reset', adminAuth, (req, res) => {
    ...
    targets.forEach(target => {
      switch (target) {
        ...
        case 'orders': {
          ...
          rows.forEach(row => {
            db.prepare('INSERT INTO archive_orders (original_id, data_json, deleted_by_email, reset_batch) VALUES (?,?,?,?)')
              .run(row.id, JSON.stringify(row), req.admin.email, batchId)
          })
        ...
        case 'expenses': {
          ...
          rows.forEach(row => {
            db.prepare('INSERT INTO archive_expenses (original_id, data_json, deleted_by_email, reset_batch) VALUES (?,?,?,?)')
              .run(row.id, JSON.stringify(row), req.admin.email, batchId)
          })
        ...
        case 'cogs': {
          ...
          rows.forEach(row => {
            db.prepare('INSERT INTO archive_cogs (original_id, data_json, deleted_by_email, reset_batch) VALUES (?,?,?,?)')
              .run(row.id, JSON.stringify(row), req.admin.email, batchId)
          })
        ...
        case 'products': {
          ...
          rows.forEach(row => {
            db.prepare('INSERT INTO archive_products (original_id, data_json, deleted_by_email, reset_batch) VALUES (?,?,?,?)')
              .run(row.id, JSON.stringify(row), req.admin.email, batchId)
          })
        ...
        case 'suppliers': {
          ...
          rows.forEach(row => {
            db.prepare('INSERT INTO archive_suppliers (original_id, data_json, deleted_by_email, reset_batch) VALUES (?,?,?,?)')
              .run(row.id, JSON.stringify(row), req.admin.email, batchId)
          })
  ...
  try {
    ...
    auditLog({
      ...
      oldValue: summary,
      newValue: { status: 'cleared', batch: batchId }
    })
    ...
    res.json({
      ...
      batch: batchId,
      summary,
      message: `Reset complete. All data archived under batch ${batchId}.`
    })
  } catch (e) {
...
// GET /api/admin/system/archives — see what's been archived
