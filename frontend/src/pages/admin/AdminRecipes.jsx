...
export default function AdminRecipes() {
  ...
  async function load() {
    ...
    try {
      ...
      setProducts(pr.data.filter(p => p.category === 'handmade'))
  ...
  function addLine() { setRecipe(r => [...r, { material_id:'', qty_needed:'' }]) }
  function removeLine(i) { setRecipe(r => r.filter((_, idx) => idx !== i)) }
  function updateLine(i, key, val) { setRecipe(r => r.map((line, idx) => idx === i ? { ...line, [key]: val } : line)) }
  ...
  return (
    <>
      <PageHeader title="Recipes" subtitle="Bill of Materials — Handmade products only" />
      <PageContent>

        <KpiGrid>
          <KpiCard label="Handmade Products" value={products.length} />
          <KpiCard label="With Recipe"        value={withRecipe} />
          <KpiCard label="Without Recipe"     value={products.length - withRecipe} />
          <KpiCard label="Avg Margin"         value={`${isNaN(avgMargin) ? 0 : avgMargin.toFixed(0)}%`} changeUp={avgMargin > 0} />
        </KpiGrid>

        {loading ? <Spinner /> : products.length === 0 ? (
          <Card>
            <Empty message="No handmade products found — add a product with category 'Handmade' first" />
          </Card>
        ) : (
          <Card>
            <Table headers={['Product', 'Sale Price', 'Recipe Cost', 'Margin', 'Materials', '']}>
              {products.map(p => {
